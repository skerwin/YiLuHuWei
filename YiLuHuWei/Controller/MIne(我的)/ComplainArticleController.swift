...
class ComplainArticleController: BaseViewController,Requestable {
    ...
    var ComplainModelList = [PerAVCModel]()
    ...
    func loadData(){
        if ComplainType == 0{
            let requestParams = HomeAPI.mineComplainCommentPathAndParams(limit: limit, page: page)
            postRequest(pathAndParams: requestParams,showHUD: true)
            
        }else{
            let requestParams = HomeAPI.mineComplainPathAndParams(limit: limit, page: page, report_type: ComplainType)
            postRequest(pathAndParams: requestParams,showHUD: true)
        }
    ...
    override func onResponse(requestPath: String, responseResult: JSON, methodType: HttpMethodType) {
        ...
        let list:[PerAVCModel]  = getArrayFromJsonByArrayName(arrayName: "list", content:  responseResult)
...
extension ComplainArticleController:UITableViewDataSource,UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        cell.ComplainType = self.ComplainType
