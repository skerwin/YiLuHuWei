...
class ComplainTypeController: BaseViewController,Requestable {
    ...
    @IBAction func sureBtnACtion(_ sender: Any) {
        ...
        if amodel == nil{
            let pathAndParams = HomeAPI.ContentComplainPathAndParams(id: model.id, table_name: "comment", title: model.content, reason: detailContent, publisher: model.user_id, categories: intType)
            postRequest(pathAndParams: pathAndParams,showHUD: false)
        }else{
            let pathAndParams = HomeAPI.ContentComplainPathAndParams(id: amodel.id, table_name: amodel.table_name, title: amodel.title, reason: detailContent, publisher: amodel.user_id, categories: intType)
            postRequest(pathAndParams: pathAndParams,showHUD: false)
        }
