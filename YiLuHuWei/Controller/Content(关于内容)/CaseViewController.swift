...
class CaseViewController: BaseViewController,Requestable {
    ...
    func loadData(){
        ...
        if menupagetype == MenuPageType.Channel {
            ...
            let pathAndParams = HomeAPI.groupShowListPathAndParams(page: page, limit: limit, id: groupID, list_type: 2)
            postRequest(pathAndParams: pathAndParams,showHUD: false)
        }
        else if menupagetype == MenuPageType.MineGood{
            self.title = "我的点赞"
            
            let pathAndParams = HomeAPI.personsLikeListPathAndParams(page: page, limit: limit, table_name: "cases")
            postRequest(pathAndParams: pathAndParams,showHUD: false)
        }
        else if menupagetype == MenuPageType.MineCare{
            self.title = "我的收藏"
            let pathAndParams = HomeAPI.favoriteListPathAndParams(page: page, limit: limit, table_name: "cases")
            postRequest(pathAndParams: pathAndParams,showHUD: false)
        }
        else if menupagetype == MenuPageType.MinePublish{
            self.title = "我的发布"
            let pathAndParams = HomeAPI.myPublishPathAndParams(page: page, limit: limit, list_type: 2)
            postRequest(pathAndParams: pathAndParams,showHUD: false)
        }
        else if menupagetype == MenuPageType.Search{
            self.title = searchKey
            let pathAndParams = HomeAPI.HomeSearchPathAndParams(keyword:searchKey,page: page, limit: limit, list_type: 2)
            postRequest(pathAndParams: pathAndParams,showHUD: false)
        }
        
        else{
            let requestParams = HomeAPI.caseListPathAndParams(page: page, limit: limit)
