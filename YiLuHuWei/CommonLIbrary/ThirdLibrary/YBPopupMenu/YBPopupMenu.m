...
+(YBPopupMenu *)sharedInstance
{
    static YBPopupMenu *sharedManager;
      
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedManager = [[YBPopupMenu alloc] init];
    });
...
- (void)showRelyOnView:(UIView *)view titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth otherSetting:(void (^) (YBPopupMenu * popupMenu))otherSetting
{
    CGRect absoluteRect = [view convertRect:view.bounds toView:YBMainWindow];
    CGPoint relyPoint = CGPointMake(absoluteRect.origin.x + absoluteRect.size.width / 2, absoluteRect.origin.y + absoluteRect.size.height);
    //YBPopupMenu *popupMenu = [[YBPopupMenu alloc] init];
    ...
    _arrowHeight = 10.0;
    _minSpace = 5.0;
    YB_SAFE_BLOCK(otherSetting,self);
    [self show];
 }

- (void)refreshIcon:(NSArray *)icons titles:(NSArray *)titles{
    [self setImages:icons];
